...
<div class="mt-2 transition-opacity duration-300 ease-in-out flex">                <input type="text" v-model="message.reply" placeholder="Reply..."
...
export default {
    ...
    methods: {
        sendReply(message) {
            ...
            let removedMessage = this.messages.filter(item => item.id === message.id)[0];
